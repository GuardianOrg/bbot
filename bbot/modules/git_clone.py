...
class git_clone(github):
    ...
    async def clone_git_repository(self, repository_url):
        owner = self.helpers.tagify(repository_url.split("/")[-2])
        if not owner:
            self.warning(f"Could not derive a safe repository owner from {repository_url}")
            return
        folder = self.output_dir / owner
        if not folder.resolve().is_relative_to(self.output_dir.resolve()):
            self.warning(f"Refusing to clone outside output directory: {folder}")
            return
        ...
        # sanitize the repo
        # Preserve secret-bearing metadata for scanning while replacing active git
        # configuration and hooks with inert equivalents.
