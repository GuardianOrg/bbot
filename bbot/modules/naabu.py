...
class naabu(BaseModule):
    ...
    async def _scan_target_group(self, target_group, correlator, emitted, emitted_lock=None):
        ...
        try:
            for command in self._build_naabu_commands(target_file, target_group):
                await self._run_naabu_command(command, target_group, ports_by_ip)
            await self._emit_limited_open_ports(ports_by_ip, correlator, emitted, emitted_lock)
        finally:
            target_file.unlink(missing_ok=True)

    async def _run_naabu_command(self, command, targets, ports_by_ip):
    ...
    async def _emit_limited_open_ports(self, ports_by_ip, correlator, emitted, emitted_lock=None):
        for ip in sorted(ports_by_ip, key=str):
            ports = sorted(ports_by_ip[ip])
            ip_hash = hash(ip)
            if len(ports) > self.max_open_ports_per_ip:
                self.open_port_cache[ip_hash] = tuple()
                self.warning(
                    f"{self.name} found {len(ports):,} open TCP ports on {ip}; discarding this module's results for that IP because the limit is {self.max_open_ports_per_ip:,}"
                )
                continue
