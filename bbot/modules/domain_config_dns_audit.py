        ...
        if not spf_records:
            if records.get("MX"):
                findings.append(AuditFinding(
                    ...
                    "Email",
                    (
                        "The domain accepts email but does not publish SPF. Mail receivers have no DNS policy describing which servers may send mail for the domain, making spoofed email harder to detect."
                        ' SPF is an email DNS policy that lists which servers are allowed to send mail using'
                        'this domain. Without SPF, receiving mail systems have less information to decide whether a message claiming to'
                        'come from the domain is legitimate. Attackers can more easily send spoofed messages that appear to use the'
                        "organization's domain, especially when other controls such as DKIM and DMARC are also missing or weak. SPF"
                        'alone does not stop all phishing, but it is a basic control that supports DMARC enforcement and improves'
    ...
    async def check_dmarc(self, domain, records, findings):
        ...
        if not dmarc:
            if records.get("MX"):
                findings.append(AuditFinding(
                    ...
                    "Email",
                    (
                        "The domain accepts email but does not publish a valid DMARC policy. Without DMARC, receivers cannot reliably reject or quarantine messages that fail SPF and DKIM alignment, making domain spoofing easier."
                        ' DMARC tells mail receivers what to do when a message claiming to be from the domain'
                        'fails SPF and DKIM alignment checks. Without DMARC, receivers may still perform SPF or DKIM checks, but they'
                        'do not have a clear domain-owner policy saying whether failed messages should be rejected, quarantined, or'
                        'only monitored. This makes direct domain spoofing easier and reduces visibility into abuse. DMARC also'
        ...
        if policy and policy.group(1) in {"none", "quarantine"}:
            findings.append(AuditFinding(
                ...
                "Email",
                (
                    f"The DMARC policy is set to p={policy.group(1)}. Messages that fail authentication are not fully rejected, so spoofed email may still reach recipients depending on receiver behavior."
                    ' DMARC enforcement controls what receivers should do with mail that fails'
                    'authentication and alignment. A monitoring-only or partial policy can be useful while legitimate senders are'
    ...
    async def check_caa(self, domain, records, findings):
        ...
        if not success or not caa_records:
            findings.append(AuditFinding(
                ...
                "Certificate",
                (
                    "The domain has no CAA records. Any publicly trusted certificate authority may issue certificates for the domain, increasing the blast radius of CA mistakes or account compromise."
                    ' CAA records tell public certificate authorities which CAs are allowed to issue TLS'
                    'certificates for the domain. Without CAA, any publicly trusted CA can issue a certificate if its normal'
                    'validation process succeeds. That does not mean certificates are currently compromised, but it increases the'
                    'number of organizations whose mistakes or account compromises could affect the domain. CAA is a governance and'
                    'damage-reduction control: it narrows the allowed issuers and can provide incident reporting for unauthorized'
